...
import { countWorkloadOccurrences, filterLogsByScope, summarizeLogsByDate, summarizeMonthlyWorkloadOccurrences } from "@/lib/work-log-insights";
...
export default function Home() {
  ...
  const monthlyStats = useMemo(() => summarizeMonthlyWorkloadOccurrences(logs, selectedDate, WORKLOADS), [logs, selectedDate]);
  ...
  const handleExportStatistics = () => {
    const workbook = buildWorkloadStatisticsExcel(selectedDate.slice(0, 7), monthlyStats);
    const blob = new Blob([workbook], { type: "application/vnd.ms-excel;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `สถิติการทำงาน-${selectedDate.slice(0, 7)}.xls`;
    link.click();
    URL.revokeObjectURL(url);
  };
