...
export function DailyHistory({ summaries, selectedDate, onSelectDate }: Props) {
  return (
    <section aria-labelledby="daily-history-title" className="rounded-3xl border border-[var(--line)] bg-white/45 p-5 sm:p-6">
      ...
      {!summaries.length ? <div className="paper-grid rounded-2xl border border-dashed border-[#c5cad5] px-5 py-10 text-center text-sm text-[var(--muted)]">ยังไม่มีวันที่บันทึกข้อมูล</div> : <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{summaries.map((summary) => {
