...
export function DailyLogDialog({ date, logs, workloads, onEdit, onDelete, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-[rgba(23,35,63,.46)] p-0 backdrop-blur-[3px] sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div role="dialog" aria-modal="true" aria-labelledby="daily-log-dialog-title" className="max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-t-3xl bg-[var(--paper)] p-5 shadow-2xl sm:rounded-3xl sm:p-7">
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4 border-b border-[var(--line)] pb-5">
          <div><p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--gold)]">รายการที่บันทึกไว้</p><h2 id="daily-log-dialog-title" className="mt-1 text-2xl font-semibold tracking-tight">{formatThaiDate(date)}</h2><p className="mt-1 text-sm text-[var(--muted)]">ทั้งหมด {logs.length} รายการ</p></div>
          <button type="button" onClick={onClose} className="focus-ring rounded-xl border border-[var(--line)] bg-white px-4 py-2.5 text-sm font-semibold text-[var(--ink)] transition hover:bg-[#eef1f8]">ปิดหน้าต่าง</button>
        </div>
